...
export default function UpdateLPModal({
  close,
  idToken,
  fetchLicensePlates,
  currentLP,
  ...
  const [LPinput, setLP] = useState(currentLP.plateNumber);
  const [vehicleType, setVehicleType] = useState(currentLP.vehicleType);
  const [LPprovince, setLPprovince] = useState({
    title: currentLP.province,
    firstLetter: "",
  });
  ...
  const submit = async () => {
    ...
    const res = await updateLicensePlate(idToken, currentLP.id, reqBody);
